...
import { BASE_URL,check_permission,ADMIN_NUMBER } from '../../config';
...
export default function List(props) {
    ...
    const deleteItem = async (id) => {
        ...
        await axios.delete(`roles/${id}`).then(({ data }) => {
            Swal.fire({
                icon: "success",
                text: data.message
            })
                                            ...
                                            informations.map((row, key) => (
                                                <tr key={key}>
                                                    ...
                                                    <td>
                                                        { row.id != ADMIN_NUMBER ? ( 
                                                            <>
                                                                <Link to={`/roles/edit/${row.id}`} className='btn btn-success me-2'>
                                                                        Edit
                                                                </Link>
